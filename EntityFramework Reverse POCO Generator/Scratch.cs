...
ORDER BY FK.TABLE_NAME, CU.COLUMN_NAME";
                ...
                using(Cmd)
                {
                    using(DbDataReader rdr = Cmd.ExecuteReader())
                    {
                        while(rdr.Read())
                        {
                            ...
                            Column pkCol = fkTable.Columns.Find(n => n.PropertyName == pkColumn);
                            if(pkCol == null)
                                continue;
                            ...
                            fkCol.EntityFk = string.Format("public virtual {0} {1} {2} {3}", pkTableHumanCase, fkName, "{ get; set; } // ",
                                                           fkCol.PropertyNameHumanCase + " - " + constraintNameHumanCase);

                            fkCol.ConfigFk = string.Format("{0}; // {1}", GetRelationship(fkCol, pkCol, fkName), constraintName);
                            //fkCol.ConfigFk = string.Format("Has{0}(a => a.{1}).With{2}.HasForeignKey(b => b.{3}); // {4}", GetHasMethod(fkCol, pkCol), fkName, GetWithMethod(fkCol, pkCol), fkCol.PropertyNameHumanCase, constraintName);
                        }
                    }
                }
            ...
            private static string GetWithMethod(Column fkCol, Column pkCol)
            {
                ...
                // 1:n
                return string.Format(".Many().HasForeignKey(b => b.{0})", fkCol.PropertyNameHumanCase);
            }
