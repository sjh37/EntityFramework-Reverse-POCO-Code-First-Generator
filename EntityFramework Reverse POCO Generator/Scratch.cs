...
ORDER BY FK.TABLE_NAME, CU.COLUMN_NAME";
                ...
                using(Cmd)
                {
                    using(DbDataReader rdr = Cmd.ExecuteReader())
                    {
                        while(rdr.Read())
                        {
                            ...
                            fkCol.ConfigFk += string.Format(" pkCol.IsPrimaryKey={0} fkCol.IsNullable={1} {2}.{3}", pkCol.IsPrimaryKey, fkCol.IsNullable, pkTable.Name, pkCol.Name);
                        }
                    }
                }
            ...
            {
                // Check multiplicity
                if(pkCol.IsPrimaryKey && !fkCol.IsNullable)
