    ...
    public class Table : EntityName
    {
            ...
            string constraint, List<ForeignKey> fks, Table mappingTable = null)
        {
            ...
            switch (relationship)
            {
                ...
                case Relationship.OneToMany:
                    var nullForgivingOneToMany = (Settings.NeedsNullForgiving()) ? " = null!" : string.Empty;
                    ReverseNavigationProperty.Add(
                        new PropertyAndComments
                        {
                            AdditionalDataAnnotations = _filter.ForeignKeyAnnotationsProcessing(fkTable, this, propName, string.Empty),
                            PropertyName = propName,
                            Definition = string.Format("{0} {1}{2} {3} {{ get; set; }}{4}{5}", accessModifier, GetLazyLoadingMarker(), fkTable.NameHumanCaseWithSuffix(), propName, nullForgivingOneToMany, Settings.IncludeComments != CommentsStyle.None ? " // " + constraint : string.Empty),
