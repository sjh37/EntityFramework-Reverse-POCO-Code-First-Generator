...
SELECT  R.SPECIFIC_SCHEMA,
        ...
        (SELECT TOP 1 sm.definition
         FROM   sys.all_objects obj2
                INNER JOIN sys.all_sql_modules sm ON sm.object_id = obj2.object_id
         WHERE  SCHEMA_NAME(obj2.schema_id) = R.SPECIFIC_SCHEMA
                AND obj2.name = R.SPECIFIC_NAME) AS PROC_DEFINITION
...
SELECT  TOP (0)
        ...
        ISNULL(P.DATETIME_PRECISION, 0) AS DATETIME_PRECISION,
        P.USER_DEFINED_TYPE_SCHEMA + '.' + P.USER_DEFINED_TYPE_NAME AS USER_DEFINED_TYPE,
        CAST(NULL AS NVARCHAR(MAX)) AS PROC_DEFINITION
...
INSERT INTO #SynonymStoredProcedureDetails (SPECIFIC_CATALOG, SPECIFIC_SCHEMA, SPECIFIC_NAME, ROUTINE_TYPE, RETURN_DATA_TYPE, ORDINAL_POSITION, PARAMETER_MODE, PARAMETER_NAME, DATA_TYPE
                                            , CHARACTER_MAXIMUM_LENGTH, NUMERIC_PRECISION, NUMERIC_SCALE, DATETIME_PRECISION, USER_DEFINED_TYPE, PROC_DEFINITION)
...
SELECT SPECIFIC_SCHEMA, SPECIFIC_NAME, ROUTINE_TYPE, RETURN_DATA_TYPE, ORDINAL_POSITION, PARAMETER_MODE, PARAMETER_NAME, DATA_TYPE
    , CHARACTER_MAXIMUM_LENGTH, NUMERIC_PRECISION, NUMERIC_SCALE, DATETIME_PRECISION, USER_DEFINED_TYPE, PROC_DEFINITION FROM #SynonymStoredProcedureDetails";
        }
