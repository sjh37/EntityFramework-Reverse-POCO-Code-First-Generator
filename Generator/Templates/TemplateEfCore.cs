...
{{#if IsEfCore3Plus}}
    ...
    {{{#newline}}
        ...
        var executionResult = typeof(IQueryProvider){{#newline}}
            .GetMethod({{#newline}}
                name: nameof(IQueryProvider.Execute),{{#newline}}
                genericParameterCount: 1,{{#newline}}
                types: new[] { typeof(Expression) }){{#newline}}
            .MakeGenericMethod(expectedResultType){{#newline}}
            .Invoke(this, new[] { expression });{{#newline}}{{#newline}}
